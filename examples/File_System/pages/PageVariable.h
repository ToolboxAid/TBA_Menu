...
public:
  ...
  template <typename T>
  static T ConvertString(const char * &name, const char * &data, bool &success)
  {
    success = true;
    if (!data  == NULL)
    {
      ...
      if (data.find("0x") != const char *::npos)
      {
        iss >> std::hex >> ret;
      }
      else
      ...
      {
        success = false;
        Serial.print("Convert failed for varName: '");
        Serial.print(name);
        Serial.print("' value: '");
        Serial.print(data);
        Serial.println("' '");
  ...
  */
  ...
  { // Do some work on a specific variables page
    ...
    variableName = "var1";
    variableValue = Menu::getInstance()->getPageVariables(variableName);
    if (!variableValue  == NULL)
    {
      unsigned short int dNum = PageVariable::ConvertString<unsigned short int>(variableName, variableValue, convert_success);
      ...
      if (convert_success)
      {
        ...
        bool oops = Menu::getInstance()->setPageVariables(variableName, done);
        if (!oops)
        {
          // Serial.println("failed to save");
          // already printed in getPageVariables.
    ...
    if (!variableValue  == NULL)
    {
      int dNum = PageVariable::ConvertString<int>(variableName, variableValue, convert_success);
    ...
    if (!variableValue  == NULL)
    {
      double dNum = PageVariable::ConvertString<double>(variableName, variableValue, convert_success);
